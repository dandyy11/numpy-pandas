...
df = pd.DataFrame(produce_dictionary, index=['Cost per Pound', 'Quantity Sold', 'Total Sales']).T


print()
print("Question 1: Produce that had the highest and lowest sales in total sales (both name of produce and value)")
highest = df['Total Sales'].sort_values(ascending=False).head(1)
lowest = df['Total Sales'].sort_values().head(1)
print("Highest sales:", highest.index[0], highest.values[0])
print("Lowest sales:", lowest.index[0], lowest.values[0])
print()


print()
print("Question 2: Using 'loc', display the quantity and total sales for 'Orange' and 'Beets' (together)")
print(df.loc[['Orange', 'Beets'], ['Quantity Sold', 'Total Sales']])
print()


print()
print("Question 3: Using 'loc', display the total sales for 'Apples' through 'Lettuce'")
print(df.loc['Apples':'Lettuce', 'Total Sales'])
print()
...
df.at['Apricots', 'Total Sales'] = 44353.05
print(df.loc['Apricots'])
print()
...
df_new = df[(df['Quantity Sold'] >= 11500) & (df['Quantity Sold'] <= 12000)]
print(df_new)
print()
